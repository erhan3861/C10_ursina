...
class Collect(Entity):
    "Mutlaka nesne tanımı sonrası collect.gui() kodunu yazınız."
    text = None
    icon = None
    count = 0
    ...
    def update(self):
        if distance_xz(self, self.player) < 1.5 and self.not_collected:
            ...
            new_collect = Entity(parent=camera.ui, model="sphere", scale=0.05, color=color.yellow, position=pos, shader=bls, unlit=True)
