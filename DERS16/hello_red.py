...
def on_data(connection, data, time_received):
    ...
    s.start()
    if "red" in data.decode("utf-8"):
        window.color = color.red
    elif "blue" in data.decode("utf-8"):
...
def input(key):
    if key == "space":
        ...
        if peer.is_running():
            peer.send(peer.get_connections()[0], "red".encode("utf-8"))

    elif key == "b":
        if peer.is_running():
            peer.send(peer.get_connections()[0], "blue".encode("utf-8"))
